...
#
<% if @debug %>SESSION_COOKIE_SECURE = False<% end %>
...
MIDDLEWARE_CLASSES = (
    ...
    # NOTE (RS) disabled because it requires Django 1.6
...
MIDDLEWARE_CLASSES =  MIDDLEWARE_CLASSES + (
    'kgauth.backends.shibboleth.middleware.FakeShibMiddleware',
)
<% end %>
...
DATABASES = {
    ...
    'rcshibboleth': {
        'ENGINE': 'django.db.backends.mysql',
        'NAME': '<%= rcshibboleth_db_name %>',
        'USER': '<%= rcshibboleth_db_user %>',
        'PASSWORD': '<%= rcshibboleth_db_pass %>',
        'HOST': '<%= rcshibboleth_db_host %>',
        'PORT': '<%= rcshibboleth_db_port %>',
        'ATOMIC_REQUESTS': True,
    }
}

LDAP_BASE = 'dc=dev,dc=rc,dc=nectar,dc=org,dc=au'
    ...
    'default' : [
        {
            ...
            'LDAP_ACCOUNT_BASE': 'ou=Accounts, %s' % LDAP_BASE,
...
SAML_PERSON_AUTOCREATE = True

SAML_INSTITUTE_AUTOCREATE = True
